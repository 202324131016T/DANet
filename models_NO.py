...
class ImageEncoder(nn.Module):
    def __init__(self, model_name=CFG.model_name, trainable=CFG.trainable):
        ...
        # 加载本地权重
        local_weight_path = "./info/resnet50_a1_0-14fe96d1.pth"
...
class SpotEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        # C_in = 32
        # self.fc_in = nn.Linear(3467, C_in)
        # self.kan = KAN(width=[C_in, C_in])
        # self.tanh = nn.Tanh()
        # self.fc_out = nn.Linear(C_in, 3467)

        self.mamba = my_mamba(dim=1, d_state=16, d_conv=4, expand=2)
    ...
    def forward(self, x):
        ...
        x = self.mamba(x.unsqueeze(2)).squeeze(2)
        return x
...
class CLIPModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        #
        # Calculating the Loss  smoothed objective == No
        logits = (spot_embeddings @ image_embeddings.T) / self.temperature
        targets = torch.eq(torch.arange(len(logits)).unsqueeze(1), torch.arange(len(logits)).unsqueeze(0))
